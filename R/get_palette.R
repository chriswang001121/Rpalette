...
get_palette <- function(name,
  ...
  if (!file.exists(palette_rds)) {
    cli::cli_alert_danger("Color palette file not found: {.path {palette_rds}}")
  ...
  if (!type %in% valid_types) {
    cli::cli_alert_danger("Invalid type '{type}', available types: {.val {valid_types}}")
    stop("Type mismatch.")
  }
  ...
  if (!name %in% names(palettes[[type]])) {
    ...
    if (is.null(found_type)) {
      ...
      cli::cli_alert_warning("Palette '{name}' not found in type '{type}', but found in '{found_type}'")
      cli::cli_alert_info("Consider changing the type parameter to '{found_type}'.")
  ...
  cli::cli_alert_success("Successfully extracted '{name}', number of colors: {.val {max_len}}")
  ...
  if (!is.numeric(n) || n != round(n) || n <= 0) {
    cli::cli_alert_danger("Parameter 'n' must be a positive integer, current value: {.val {n}}")
